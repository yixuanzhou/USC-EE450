...
void tcp_buyer_client(int port) {
    ...
    printf("<Buyer2> has TCP port %d and IP address %s for Phase I part 4\n", portnum, ipaddr);
}

void respond_to_agent(string seller_name, int budget, int agent_num) {
    char buf[256];
    string response = "Buyer2,"+seller_name+","+toString(budget);
    //cout << response << endl;
    printf("<Buyer2> has sent <%s> to <Agent%d>\n", response.c_str(), agent_num);
...
void udp_buyer_server(int port) {
    ...
    udp_servaddr.sin_port = htons(BUYER2_UDP_PORT);
    ...
    cout << "The <Buyer2> has UDP port " << BUYER2_UDP_PORT << " and IP address " << inet_ntoa(*(struct in_addr*)he->h_addr) << " for Phase 1 part 2" << endl;
...
void create_tcp_server() {
    ...
    tcp_servaddr.sin_family = AF_INET;
    tcp_servaddr.sin_addr.s_addr = INADDR_ANY;
    tcp_servaddr.sin_port = htons(Buyer2_TCP_PORT);
    ...
    cout << "<Buyer2> has TCP port " << Buyer2_TCP_PORT << " and IP address " << ipaddr << " for Phase I part 4" << endl;
}
...
void receive_from_agent() {
    ...
    sleep(1);
    cout << "End of Phase I part 4 for <Buyer2>" << endl;
}

 
int main() {
    udp_buyer_server(BUYER2_UDP_PORT);
    ...
    cout << "End of Phase I part 3 for <Buyer2>" << endl;
    sleep(1);
