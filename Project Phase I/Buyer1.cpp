...
void tcp_buyer_client(int port) {
    ...
    printf("<Buyer1> has TCP port %d and IP address %s for Phase I part 4\n", portnum, ipaddr);
}

void respond_to_agent(string seller_name, int budget, int agent_num) {
    char buf[256];
    string response = "Buyer1,"+seller_name+","+toString(budget);
    //cout << response << endl;
    printf("<Buyer1> has sent <%s> to <Agent%d>\n", response.c_str(), agent_num);
...
void udp_buyer_server(int port) {
    ...
    cout << "<Buyer1> has UDP port " << Buyer1_UDP_PORT << " and IP address " << inet_ntoa(*(struct in_addr*)he->h_addr) << " for Phase 1 part 2" << endl;
}
...
int main() {
    ...
    cout << "End of Phase I part 3 for <Buyer1>" << endl;
    sleep(1);
