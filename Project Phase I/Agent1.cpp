...
void tcp_server(int port, int part) {
    ...
    tcp_servaddr.sin_port = htons(Agent1_TCP_PORT);
    ...
    printf("<Agent1> has TCP port %d and IP address %s for Phase I part 1\n", Agent1_TCP_PORT, inet_ntoa(*(struct in_addr*)he->h_addr));
...
void receive_from_sellers(int numOfClients) {
    ...
    sleep(1);
    cout << "End of Phase I part 1 for <Agent1>" << endl;
}
...
void receive_from_buyers(int numOfBuyers) {
    ...
    if ((he = gethostbyname("nunki.usc.edu")) == NULL) perror("gethostbyname");
    printf("<Agent1> has TCP port %d and IP address %s for Phase I part 3\n", Agent1_TCP_PORT, inet_ntoa(*(struct in_addr*)he->h_addr));
...
vector< vector <string> > calculate_bid() {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (bid[1] == "sellerA") {
            ...
            if (curr_bid > bidA) { // if current budget is larger than max budget
                res[0] = bid[0];  // update seller with this budget
                bidA = curr_bid;  // update max budget
            }
        }
        if (bid[1] == "sellerB") {
            int curr_bid = atoi(bid[2].c_str());
            interested_buyers.push_back(bid[0]);
            if (curr_bid > bidB) { // if current budget is larger than max budget
                res[1] = bid[0];  // update seller with this budget
                bidB = curr_bid;  // update max budget
            }
        }
    }
...
void send_final_result(int port, string name, string result) {
    ...
    printf("<Agent1> has sent the result to <%s>\n", name.c_str());
    sleep(1);
}
...
void get_client_tcpip() {
    ...
    printf("<Agent1> has TCP port %d and IP address %s for Phase I part 4\n", ntohs(tcp_cliaddr.sin_port), ipaddr);
}
...
int main(int argc, char *argv[]) {
    ...
    /* Iterate through all to_buyers message, if the buyer want to buy house from the seller
       who is connected with Agent1, then send result to this buyer */
    ...
    cout << "End of Phase I part 4 for <Agent1>" << endl;
