...
#define BUYER3_UDP_PORT 21976
#define Buyer3_TCP_PORT 4476
...
void tcp_buyer_client(int port) {
    ...
    printf("<Buyer3> has TCP port %d and IP address %s for Phase I part 4\n", portnum, ipaddr);
}

void respond_to_agent(string seller_name, int budget, int agent_num) {
    char buf[256];
    string response = "Buyer3,"+seller_name+","+toString(budget);
    //cout << response << endl;
    printf("<Buyer3> has sent <%s> to <Agent%d>\n", response.c_str(), agent_num);
...
void udp_buyer_server(int port) {
    ...
    udp_servaddr.sin_port = htons(BUYER3_UDP_PORT);
    ...
    cout << "The <Buyer3> has UDP port " << BUYER3_UDP_PORT << " and IP address " << inet_ntoa(*(struct in_addr*)he->h_addr) << " for Phase 1 part 2" << endl;
...
map <string, vector <int> > receive_from_agent(int numOfAgents) {
    ...
    cout << "End of Phase I part 2 for <Buyer3>" << endl;
    close(udp_sockfd);
...
void create_tcp_server() {
    ...
    tcp_servaddr.sin_family = AF_INET;
    tcp_servaddr.sin_addr.s_addr = INADDR_ANY;
    tcp_servaddr.sin_port = htons(Buyer3_TCP_PORT);
    ...
    cout << "<Buyer3> has TCP port " << Buyer3_TCP_PORT << " and IP address " << ipaddr << " for Phase I part 4" << endl;
}
...
void receive_from_agent() {
    ...
    sleep(1);
    cout << "End of Phase I part 4 for <Buyer3>" << endl;
}
 
int main() {
    udp_buyer_server(BUYER3_UDP_PORT);

    receive_from_agent(2);
    string seller_list[] = {"sellerA", "sellerB", "sellerC", "sellerD"};

    vector<int> buyer_info = readFile("buyer/buyer3.txt");    
    ...
    cout << "End of Phase I part 3 for <Buyer3>" << endl;
    sleep(1);
